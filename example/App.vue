<template>
  <div id="app">
    <github
      ...
      fill="white"
      slug="katalonne/vue-sailor-icons">
    </github>
    <header class="header">
      <div class="container">
        <h1 class="hero-heading">vue-sailor-icons</h1>
    ...
    <div class="container">
      ...
      <div class="icons">
        <div
          ...
          :key="icon">
          <component :is="icon" width='40' height='40' color="#2c55a0" class="icon-svg"></component>
          <span>{{ icon }}</span>
        </div>
      </div>
      <div class="sizing">
        ...
        <div class="sizes">
          <div
            ...
            <div class="size-icon">
              <anchor-icon color="#2c55a0" :size="size"></anchor-icon>
            </div>
          </div>
        </div>
      </div>
    ...
    <footer class="footer">
      <div class="container">
        &copy; {{ year }} by <a href="https://github.com/katalonne">Katalonne</a>
      </div>
    </footer>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      exampleSizes: ['40', '2x', '2.5x', '3x', '4x', '5x']
    }
  },
  computed: {
    filteredIcons() {
  ...
  methods: {
    ...
    sizeExample(size) {
      return example
        .replace('1.5x', size)
        .replace(/ICON/g, 'AnchorIcon')
        .replace(/kebab-icon/g, 'anchor-icon')
    }
  },
  components: {
    ...icons,
...
<style scoped>
.header {
  background: linear-gradient(90deg, #2c55a0, #4894ff 70%, #acc6e6);
  padding: 40px 0;
}
...
.hero-heading {
  ...
  font-size: 2rem;
  font-weight: 600;
  background: -webkit-linear-gradient(135deg, #fff9b0, #00d8ff);
